...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs > 3 || nrhs < 1) 
        mexErrMsgIdAndTxt( "MATLAB:PDA_mex:invalidNumInputs",
                "Usage : sol = PDA_mex(par, opt, verbose)");
    if(nrhs > 1 && !mxIsStruct(prhs[1]))
        mexErrMsgIdAndTxt( "MATLAB:PDA_mex:inputNotStruct",
                "Second input must be a structure.");
    if(nlhs > 1)
        mexErrMsgIdAndTxt( "MATLAB:PDA_mex:maxlhs",
                "Usage : sol = PDA_mex(par, opt, verbose)");

   // Get options if specified
    Opt opt = {1e-4, 1e-4, 2, 10};
    ...
    if (nrhs > 2) { // Verbose mode
      printf("Solving with ADMM using parameters:\n");
      printf("  primalTol         = %e\n", opt.primalTol);
      printf("  dualTol           = %e\n", opt.dualTol);
      printf("  MAXITR            = %i\n", opt.MAXITR);
...
//  double solve_time = split_toc() / (double)number_of_solves;
  ...
  const char *fnames[] = {"primal", "dual","y_solve", "itr", "rDual", "rPrimal", "solve_time_ns"};
  int nFields = 6;
 ...
 // mxArray *y_sol = mxCreateDoubleMatrix(nPrimal, 1, mxREAL);  
 // memcpy(mxGetPr(y_sol), sol.y_sol, sizeof(double)*nPrimal);
 // mxSetField(plhs[0], 0, "y_sol", y_sol);
