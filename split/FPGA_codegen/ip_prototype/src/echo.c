			...
			if (packet_type==1) //reset IP
					...
					else if (packet_type==3) //write data DDR
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //pl_vec_in_in
							if (DEBUG)
									printf("write pl_vec_in_in\n\r");

							if (FLOAT_FIX_PL_VEC_IN_IN==1) {
								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,PL_VEC_IN_IN_FRACTIONLENGTH));
								}
								memcpy(pl_vec_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(pl_vec_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
							
							case 1: //pl_mat_in_in
							if (DEBUG)
									printf("write pl_mat_in_in\n\r");

							if (FLOAT_FIX_PL_MAT_IN_IN==1) {
								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,PL_MAT_IN_IN_FRACTIONLENGTH));
								}
								memcpy(pl_mat_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(pl_mat_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
					...
					else if (packet_type==4) //read data from DDR
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //pl_vec_out_in
							if (DEBUG)
								printf("read pl_vec_out_out\n\r");
							memcpy(outvec_fix,pl_vec_out_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							for (i=0; i<ETH_PACKET_LENGTH_RECV-2; i++)
							{
								if (FLOAT_FIX_PL_VEC_OUT_OUT==1) { //fixed-point
									tmp_float=((float)outvec_fix[i])/pow(2,PL_VEC_OUT_OUT_FRACTIONLENGTH);
									outvec[i]=*(Xint32*)&tmp_float;
								} else { //floating point
