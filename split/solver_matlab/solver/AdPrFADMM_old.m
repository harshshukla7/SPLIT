function [sol, stats_out, stats_plot] = AdPrFADMM(prob, settings)
%
% Quick implementation of ADMM with optional scaling, Nesterov
% acceleration, restart and adaptive stepsize
%
% Solves the problem
...
yd = settings.primal_vars_y;    hat.yd = yd;
if ~isfield(settings, 'dual_vars_lam'),     settings.dual_vars_lam = zeros(size(L,1),1); end
lam = settings.dual_vars_lam;    hat.lam = lam;

% -----------------------
% Main
% -----------------------
c = norm(L*x+l-yd)^2; % residual initialization (scaled)
iii = 0;
for jjj = 1  : settings.maxItr
    ...
    if isfield(settings, 'adapt')
        ...
        x = (X * ((rho1*D1+rho2*eye(n)) \ X'))* (rhsKKTconst + rhsKKTvar*(hat.lam/rho - hat.yd));
    else
        
        %         disp('rho is')
        %         rho
        % workdual_tp = rho*(-l + hat.yd) - hat.lam
        % hat.yd
        %
        %
        %       kkt_rhs_hs = rhsKKTconst + rhsKKTvar * (lam/rho - yd)
        %
        %x_hs_tp =  Pkkt*(Lkkt'\(Dkkt\(Lkkt\(Pkkt'*kkt_rhs_hs))))
        t = const_vector + matrix * (hat.lam/rho - hat.yd);
        x = t(1:n);
    end
    ...
    %% step 4: Convergence check on scaled
    % L, l scaled, z, lam unscaled, yd scaled
    ...
    %% step 5: adaptive restart of Nesterov's relaxation
    prev.c = c;
...
%         disp('now previous alhpha calculation')
%         (prev.alpha-1)/alpha
%         
        hat.yd = yd + (prev.alpha-1)*(yd - prev.yd)/alpha;
        ...
        hat.yd = prev.yd;
        c = 1/eta * prev.c;
%         display('nope nope nope')
    end
    
    % adaptive stepsize
    if isfield(settings, 'adapt')
        if rPrimal > 10*rDual; rho = 2*rho;
        elseif rDual > 10*rPrimal; rho = 0.5*rho;
        end
    end
