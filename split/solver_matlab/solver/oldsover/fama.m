function [sol, stats_out, stats_plot, data_to_export] = fama(prob, settings)
%
...
% eigenvalue_L = sqrt(max(eig(L'*L))); % 2-norm of L
eigenvalue_LL = max(eig(L'*L)); % |L|_2^2

if ~isfield(settings, 'rho'),     settings.rho = sigma/eigenvalue_LL; end
rho = settings.rho;
...
lam_hat = lam;
data_to_export.lam = lam; data_to_export.lam_hat = lam_hat;
...
%for jjj = 1:1
  ...
  t = KKT \ (rhsKKTconst + rhsKKTvar*lam_hat);
  x = t(1:n);
  
  % Step 2 : compute prox operators
  q = L*x + l - 1/rho*lam_hat;
...
data_to_export.prev_lam       = zeros(sum(nProxVars),1);
% data_to_export.lam_hat        = zeros(sum(nProxVars),1);
