...
static double lambda[nDual]; // May bot be used delete after words
static double nu[nEqCon]; // May bot be used delete after words
static double lam_nu [nDual + nEqCon];
static double prev_lambda[nDual]; // May bot be used delete after words
//static double prev_y[nDual]; 
static double prev_nu[nEqCon]; // May bot be used delete after words
static double prev_lam_nu [nDual + nEqCon];
...
{  
  ...
  zero_vector(xbar, nPrimal);
  //zero_vector(y, nDual);
  zero_vector(lambda, nDual); // May bot be used delete after words
  zero_vector(nu, nEqCon);    // may not be used delete after words
  zero_vector(lam_nu, nDual + nEqCon);
  
  zero_vector(workDual, nDual);
  
  
  zero_vector(prev_x, nPrimal);
  zero_vector(prev_xbar, nPrimal);
  //zero_vector(prev_y, nDual);  
  zero_vector(prev_lambda, nDual); // May bot be used delete after words
  zero_vector(prev_nu, nEqCon);    // May bot be used delete after words
  zero_vector(prev_lam_nu, nDual + nEqCon);
...
//}
  ...
  for(itr=0; itr< opt->MAXITR; itr++) 
  {
    ...
    copy_vector(prev_nu, nu, nEqCon);
    copy_vector(prev_lam_nu, lam_nu, nDual + nEqCon);
