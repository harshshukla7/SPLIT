...
(
    ...
    LDL_int Ap [ ],	/* input of size n+1, not modified */
    LDL_int Ai [ ],	/* input of size nz=Ap[n], not modified */
    real Ax [ ],	/* input of size nz=Ap[n], not modified */
    ...
    real Lx [ ],	/* output of size lnz=Lp[n], not defined on input */
    real D [ ],	/* output of size n, not defined on input */
    real Y [ ],	/* workspace of size n, not defn. on input or output */
...
(
    ...
    real X [ ],	/* size n.  right-hand-side on input, soln. on output */
    LDL_int Lp [ ],	/* input of size n+1, not modified */
    LDL_int Li [ ],	/* input of size lnz=Lp[n], not modified */
    real Lx [ ]	/* input of size lnz=Lp[n], not modified */
)
{
    LDL_int j, p, p2 ;
    for (j = 0 ; j < n ; j++)
...
(
    ...
    real X [ ],	/* size n.  right-hand-side on input, soln. on output */
    real D [ ]	/* input of size n, not modified */
)
{
    LDL_int j ;
    for (j = 0 ; j < n ; j++)
...
(
    ...
    real X [ ],	/* output of size n. */
    real B [ ],	/* input of size n. */
