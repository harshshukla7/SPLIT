function [sol, stats_out, stats_plot, data_to_export] = ama(prob, settings)
%
% Quick implementation of FAMA to test the export from the split user
% interface
%
% Solves the problem
...
for jjj = 1:settings.maxItr
  ...
  t = const_vector + matrix * lam_hat; %fama step
  
  %rank(full(KKT))
  %Pre_mult_KKT=KKT*t
  
  %post_inverse=inv(KKT)*Pre_mult_KKT
  ...
  q =  Lx_relax + l - 1/rho * lam_hat;  % fama step
