classdef splitProb < handle
  ...
  methods (Static)
    function flat = flatten(dat)
      ...
      ind = cellfun(@(x) strcmp(class(x), 'quadraticFunc') || strcmp(class(x), 'splitvar'), obj.funcs);
      ...
      % If quadObj is linear, then wrap it in a quadratic
      if strcmp(class(quadObj), 'splitvar')
        n = splitProb.numVars;
        H = spalloc(n+1,n+1,n);
        B = quadObj.basis;
        H(:,1) = H(:,1) + B/2;
        H(1,:) = H(1,:) + B'/2;
        quadObj = quadraticFunc(H);
      end
    ...
    function prob = genProbData(flat)
      ...
      % Convert to a useful form for code-gen
      
      prob = splitProb.processForCoder(prob);
    end
    ...
    function prob = processForCoder(prob)
      % parseProb
      %
      % Post-process the problem into a form that easier to deal with for
      % code-generation
      %
      ...
      if ~any(isfield(dat, {'pF','pB','pL'}))
        disp('Can only generate solvers for paramertic problems. Press enter to continue.')
        pause
        return
      end
