function [sol, stats_out, stats_plot] = PDAbacktrck(prob, settings)
%
...
if isfield(settings,'adapt')
    if strcmp(settings.adapt, 'Backtrack')
        ranvec = randn(n);
        settings.tau = sqrt(2*norm(ranvec) / norm(K'*K*ranvec));
        tau = settings.tau;
        settings.sigma = tau;
        sigma = settings.sigma;
    elseif isfield(settings.adapt, 'Fixed')
        settings.tau = 0.95/(Mu1+Mu2);
        tau = settings.tau;
        settings.sigma = 0.95/(tau*(Mu1+Mu2)^2);
        sigma = settings.sigma;
    end
...
for jjj = 1:settings.maxItr
  ...
  % step 1
  x = (eye(size(dat.Q))+tau*prec.T*dat.Q) \ (x_prev-tau*prec.T*(L'*pd+mud+dat.f'));
  x_bar = 2*x - x_prev;
...
%          proxWeight(i) = sigma*prec.P( proxInd{i}, proxInd{i}) * sqrt(prox(i).dat.c); 
   ...
   if ~isempty(dat.A)
      mud_prev    = mud;
      mud = sigma*prec.P(end-n+1:end, end-n+1:end)*pinv(full(dat.A))*(dat.A*(prec.P(end-n+1:end, end-n+1:end)\(mud/sigma)+x_bar)-dat.b);
  else
      mud_prev = zeros(n, 1);
      mud      = zeros(n, 1);
  end
...
% %   end
  ...
  dx = x-x_prev;
  dp = -prec.E(1:size(L,1),1:size(L,1))'\(pd_prev-pd);
  r1 = (1/tau)*prec.T\((-dx)) + L'*(prec.E(1:size(L,1),1:size(L,1))'\(pd - pd_prev));
  s1 = (1/sigma)*(prec.P(1:size(L,1),1:size(L,1))\-dp) + L*dx;
  if ~isempty(dat.A)
    dmu = prec.E(size(L,1)+1:size(L,1)+n,size(L,1)+1:size(L,1)+n)'\(mud-mud_prev);
    r2 = dmu;
    s2 = (1/sigma)*(prec.P(size(L,1)+1:size(L,1)+n,size(L,1)+1:size(L,1)+n)\-dmu) + dx;
    s = [s1; s2];
    r = r1 + r2;
  else
    s = s1;
  ...
  if strcmp(settings.adapt, 'Backtrack')
      if ~isempty(dat.A)
           dmud = mud - mud_prev;
           b = ( 2*tau*sigma * dx' * K' * (prec.E\[dpd; dmud]) ) / ...
          ( gamma*sigma*dx'*(prec.T\dx) + gamma*tau*(prec.E\[dpd; dmud])'*(prec.P\(prec.E\[dpd; dmud])) );
      else
           b = ( 2*tau*sigma * dx' * K' * dp ) / ...
          ( gamma*sigma*dx'*(prec.T\dx) + gamma*tau*dp'*(prec.P\dp) ); 
      end
      if b > 1
          tau = beta*tau/b;
          sigma = beta*sigma/b;
