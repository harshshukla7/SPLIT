...
settings.Lin_Solve = 'auto_SoC'; % use sparse ldl factorization
settings.FPGA_SoC = 1;
settings.paral = 30;
...
settings_fpga.x0 = z0;
settings_fpga.resi_tol = [1e-2, 1e-2, 200 , 198];
settings_fpga.mat_row = size(prob.dat.A,1) + size(prob.dat.A,2);
settings_fpga.mat_col = size(prob.dat.A,1) + size(prob.dat.A,2);

SOC_template_generator(nParam, nPrimal, nDual, settings_fpga);
% SPLIT_template
