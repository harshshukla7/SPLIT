/*
 * Mex file to interface to generated SPLIT AMA
 */
...
#define mex_interface

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    /* Check for proper number of input and output arguments */
    if (nrhs > 3 || nrhs < 1) 
        mexErrMsgIdAndTxt( "MATLAB:ama_mex:invalidNumInputs",
                "Usage : sol = admm_mex(par, opt, verbose)");
    if(nrhs > 1 && !mxIsStruct(prhs[1]))
        mexErrMsgIdAndTxt( "MATLAB:ama_mex:inputNotStruct",
                "Second input must be a structure.");
    if(nlhs > 1)
        mexErrMsgIdAndTxt( "MATLAB:admm_mex:maxlhs",
                "Usage : sol = ama_mex(par, opt, verbose)");
   ...
   // Get options if specified
    ...
    if (nrhs > 2) { // Verbose mode
      printf("Solving with AMA using parameters:\n");
      printf("  primalTol         = %e\n", opt.primalTol);
      printf("  dualTol           = %e\n", opt.dualTol);
      printf("  MAXITR            = %i\n", opt.MAXITR);
...
//         mexErrMsgIdAndTxt( "MATLAB:ama_mex:invalidInputSize",
