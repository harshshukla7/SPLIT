function [sol, stats_out, stats_plot] = templateFAMA(dat, dxs, dus, xinit, settings)
%
% Implementation of AMA with Nesterov acceleration and restart
...
% data for linear system solve (step 1)
...
for jjj = 1:settings.maxItr
  ...
  if isfield(settings,'restart')
      ...
      if test > 0 % monotonicity test
          ...
          sol = M \ b;
          z = sol(1:n*(N+1)+m*N);
          q =  -L*z + l + (1/rho*hat.lam);
          y = max(q,0); 
          prev.lam = hat.lam;
          lam = hat.lam + rho*(-L*z + l - y);    
      end
  end
