...
sys = model_generator_juan(N_carts);
...
delta = splitvar(N_carts,ttt+1);
obj = 0; % set objective to zero

% quadratic cost up to horizon N
for j = 1:ttt
    obj = obj + 0.5*x(:,j)'*Q*x(:,j)+0.5*u(:,j)'*R*u(:,j)  + 0.5*delta(:,j)'*sigma_2*delta(:,j) + sigma_1'*delta(:,j); %  %   + alpha{iii,ttt}(1)*norm(u(:,j),1)+alpha{iii,ttt}(2)*norm(x(:,j),2);
end

% last cost 
obj = obj + 0.5*x(:,ttt+1)'*Q*x(:,ttt+1) + 0.5*delta(:,ttt+1)'*sigma_2*delta(:,ttt+1) ;  %+alpha{iii,ttt}(2)*norm(x(:,ttt+1),2);
...
settings.FPGA_PL = 1;
settings_fpga.x0 = z0;
 settings.resi_tol = [1e-2, 1e-2, 180 , 1];
