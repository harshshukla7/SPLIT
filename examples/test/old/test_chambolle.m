...
switch NO
    ...
    case 1 %% l(T*z+t) = \delta+(T*z+t)
        ...
        for jjj = 1:MAXITER 
            ...
            %% step 2
            old.z = z;
            z = (H+eye(size(H,1))*(1/tau)) \ ((1/tau)*z-(K'*[p;nu]));
    ...
    case 2 %% l(T*z+t) = |Tz+t|_2 <= c'z+d
        ...
        for jjj = 1:MAXITER
            ...
            p = proj_secondOrderCone([-tilde.p(end);tilde.p(1:end-1)]);
            p = [p(2:end,1); -p(1,1)];
    ...
    case 9
        ...
        for jjj = 1:MAXITER 
            ...
            old.p = p;e
            old.nu = nu;
            tilde.p = p + sigma*(sqrtm(Q)*(bar.z-z0));
            nu = nu + sigma*(A*bar.z-b);
            
            p = prox_norm(tilde.p,sqrt(l)*sigma,2);
        ...
        cvx_solver 'sedumi'
            ...
            subject to 
            A*Z == b;
            quad_form(Z,Q,-2*Q*z0,z0'*Q*z0) <= l;
        cvx_end
        fprintf('Chambolle solution\n'); disp(z)
        fprintf('CVX solution\n'); disp(Z)
        fprintf('No. of iterations needed:\t'); disp(jjj)
        fprintf('Inequality constraints active:\t');
        if (norm(sqrtm(Q)*Z,2) - sqrt(l) <= 1e-8) 
            disp('YES') 
        else disp('NO') 
        end
