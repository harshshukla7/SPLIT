...
{   
	 ...
	 for(i = 0;i < maxiter; i++)
	 {
         ...
         compute_array_x_varying_stepsize(ktr,  ktc,  matrixKT,
                                          ktir, ktic, matrixKTI,
                                          p_nu_len, arrayPnu, x_len, arrayX, 
                                          f_len, arrayF, diagQ_len, arrayDiagQ, tau);   

         // step 4a: modification of stepsizes: chambolle2 specific
         theta = 1 / sqrt(1+2*gamma*tau);
         tau = theta*tau;
         sigma = sigma / theta;
         
         // step 4b: compute array xbar
