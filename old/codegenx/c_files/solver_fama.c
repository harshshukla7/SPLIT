                ...
                int maxiter, int num_opt_var, double stepsize, double dualtol, double alpha, double *runtime)
{
	 ...
	 for(i = 0;i < maxiter; i++)
	 {
		 ...
		 // copy y into prev_y and lam into lam_prev
    	 ...
    	 subtract_vectors(temp_array, arrayLam, arrayPrevLam, lam_len);
         scalar_vector_product(temp_array, beta - 1, lam_len);
		 vector_scalar_division(temp_array, beta_k, lam_len);      
		 add_vectors(arrayLamHat, arrayLam, temp_array, lam_len);
         ...
         // Giorgos told me, we do not need it, hence I do not use it
