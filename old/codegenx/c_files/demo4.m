...
obj = obj + x(:,end)'*x(:,end) + 2.5*norm(x(:,1), Inf);
obj = obj + 4*norm(x(:,3), 1) + 3.5*norm(x(:,2), 2) ;
...
con = con + set( -4 <= yal_x <= 4) + set(-3 <= yal_u <= 3);
con = con + set( norm(0.5*yal_x(:,2) + 2, 2) + yal_x(1,2)/4 <= 10*yal_x(2,2) );
con = con + set( 5*norm(yal_u(:,3),2) <= 1);
...
yal_obj = yal_obj + yal_x(:,end)'*yal_x(:,end) + 2.5*norm(yal_x(:,1), Inf);
yal_obj = yal_obj + 4*norm(yal_x(:,3), 1) + 3.5*norm(yal_x(:,2), 2);
